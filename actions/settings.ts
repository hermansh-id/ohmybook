...
export const updateReadingGoalAction = createServerFn({ method: "POST" })
  .validator((d: { year: number; targetBooks: number; targetPages: number }) => d)
  .handler(async ({ data }) => {
    try {
...
export const exportReadingLogToCsvAction = createServerFn({ method: "GET" })
  .handler(async () => {
    try {
...
export const getSettingsDataAction = createServerFn({ method: "GET" }).handler(async () => {
  const { db } = await import("@/lib/db");
  const { readingGoals } = await import("@/lib/db/schema");
  const { eq } = await import("drizzle-orm");
  const currentYear = new Date().getFullYear();
  const [goal] = await db.select().from(readingGoals).where(eq(readingGoals.year, currentYear)).limit(1);
  return { goal: goal || null, currentYear };
});
