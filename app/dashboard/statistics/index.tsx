...
import { useStatistics } from "@/lib/queries/statistics";
...
function StatisticsPage() {
  ...
  const readingStats = (stats as any)?.[0] || {
    totalBooksRead: 0,
    totalPagesRead: 0,
    totalBooksReading: 0,
  ...
  const yearGoal = (goal as any)?.[0] || { targetBooks: 52, currentBooks: 0 };

  const totalReadingMinutes = (sessions as any[] ?? []).reduce((sum: number, s: any) => sum + (s.session.minutesRead || 0), 0);
  const totalPagesFromSessions = (sessions as any[] ?? []).reduce((sum: number, s: any) => sum + (s.session.pagesRead || 0), 0);
  const avgPagesPerSession = (sessions as any[] ?? []).length > 0 ? Math.round(totalPagesFromSessions / (sessions as any[]).length) : 0;

  const topAuthors = (authors as any[] ?? []).filter((a) => a.booksRead > 0).sort((a, b) => b.booksRead - a.booksRead).slice(0, 10);
  const topGenres = (genres as any[] ?? []).filter((g) => g.booksRead > 0).sort((a, b) => b.booksRead - a.booksRead).slice(0, 10);
  ...
  const monthlyForYear = monthlyStats as any[] ?? [];
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-bold">{(streaks as any)?.currentStreak ?? 0} <span className="text-base font-normal text-muted-foreground">days</span></p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-bold">{(streaks as any)?.bestStreak ?? 0} <span className="text-base font-normal text-muted-foreground">days</span></p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-bold">{(streaks as any)?.totalReadingDays ?? 0} <span className="text-base font-normal text-muted-foreground">days</span></p>
              </div>
        ...
        <CardContent>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
            <div className="space-y-1">
              ...
              <p className="text-3xl font-bold">{((libraryStats as any)?.totalBooks ?? 0).toLocaleString()}</p>
            </div>
            <div className="space-y-1">
              <div className="flex items-center gap-1.5">
              ...
              <p className="text-3xl font-bold">{((libraryStats as any)?.totalAuthors ?? 0).toLocaleString()}</p>
            </div>
            <div className="space-y-1">
              <div className="flex items-center gap-1.5">
              ...
              <p className="text-3xl font-bold">{((libraryStats as any)?.totalGenres ?? 0).toLocaleString()}</p>
            </div>
            <div className="space-y-1">
              <div className="flex items-center gap-1.5">
              ...
              <p className="text-3xl font-bold">{((libraryStats as any)?.totalPages ?? 0).toLocaleString()}</p>
            </div>
            <div className="space-y-1">
              <div className="flex items-center gap-1.5">
              ...
              <p className="text-3xl font-bold">{((libraryStats as any)?.avgPages ?? 0).toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-semibold">
                {(libraryCompletion as any)?.booksRead} / {(libraryCompletion as any)?.totalBooks} books ({(libraryCompletion as any)?.percentage}%)
              </span>
            </div>
            <div className="h-3 overflow-hidden rounded-full bg-secondary">
              <div
                className="h-full bg-primary transition-all"
                style={{ width: `${Math.min((libraryCompletion as any)?.percentage ?? 0, 100)}%` }}
              />
            </div>
            {(libraryCompletion as any)?.estimatedCompletionMonths != null && (
              <p className="text-xs text-muted-foreground">
                At your current pace, ~{(libraryCompletion as any)?.estimatedCompletionMonths} months to finish the rest
              </p>
            )}
          </div>
        </CardContent>
      ...
      <ReadingHistoryChart
        data={(readingHistory as any[] ?? []).filter(r => r.period !== null) as { period: string; booksRead: number; pagesRead: number }[]}
      ...
      <Card>
        ...
        <CardContent>
          {(topRated as any[] ?? []).length === 0 ? (
            ...
            <div className="space-y-3">
              {(topRated as any[]).map((item, index) => (
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-bold">{(quoteStats as any)?.totalQuotes ?? 0}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-bold">{(quoteStats as any)?.favoriteQuotes ?? 0}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-bold">{(quoteStats as any)?.booksWithQuotes ?? 0}</p>
              </div>
