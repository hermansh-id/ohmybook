...
import { Tag, Star, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react";

type GenreWithStats = {
  genre: { genreId: number; genreName: string; description?: string | null };
  totalBooks: number;
  totalPages: number;
  booksRead: number;
...
const columnHelper = createColumnHelper<GenreWithStats>();

export function GenresClient() {
  const { data: genres = [], isLoading } = useGenres();
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState("");

  const totalBooks = useMemo(
    () => genres.reduce((sum, g) => sum + g.totalBooks, 0),
    [genres]
  );
  ...
  return (
    <div className="flex flex-col gap-4 p-4 md:gap-6 md:p-6">
      <div>
        ...
        <p className="text-muted-foreground">Browse genres in your collection</p>
      </div>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold">
              {genres.length > 0
                ? (totalBooks / genres.length).toFixed(1)
                : 0}
            </p>
          </CardContent>
        </Card>
          ...
          {genres.length === 0 ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between mt-4">
                <p className="text-sm text-muted-foreground">
                  {table.getFilteredRowModel().rows.length} genre(s)
                </p>
